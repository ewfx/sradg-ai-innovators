...
logging.basicConfig(
    filename= config['paths']['log_file_name'].format(timestamp=timestamp1),
    level=logging.INFO,
...
async def realtime_anomaly(data: RealtimeData, background_tasks: BackgroundTasks):
    try:
        ...
        anomalies_df, output_path = process_data(df)
...
async def batch_anomaly(data: list[RealtimeData], background_tasks: BackgroundTasks):
    try:
        ...
        anomalies_df, output_path = process_data_with_batch_llm(df)
